...
class GuerreDesDes:
    ...
    def attaquer(self, attaquant, defenseur):
        ...
        afficher(force_attaquant)

        afficher("Défenseur: ")
        force_defenseur = defenseur.lancer_des()
        afficher(force_defenseur)
    ...
    def attaque_succes(self, attaquant, defenseur):
        ...
        liste_des=[]
        for de in attaquant.des:
            liste_des.append(de)
        attaquant.remplacer_des([liste_des.pop(0)])
        defenseur.remplacer_des(liste_des)
    ...
    def attaque_echec(self, attaquant):
        ...
        afficher("ÉCHEC")
        # VOTRE CODE ICI
        attaquant.remplacer_des([attaquant.des.pop(0)])
    ...
    def fin_du_tour(self, joueur):
        ...
        # VOTRE CODE ICI
        liste_des = []
        taille= range(self.carte.taille_plus_grand_territoire(joueur))
        for i in taille:
            liste_des.append(De())
        joueur.ajouter_n_des(liste_des)
    ...
    def conquete(self, joueur):
        ...
        for case in cases:
            if cases[case].appartenance != joueur:
                valeur = False
        return valeur