...
class Carte:
    ...
    def obtenir_cases_joueur(self, joueur):
        ...
        for case in self.cases:
            if joueur ==self.cases[case].appartenance:
                coordonnees_joueur[case]=self.cases[case]
        return coordonnees_joueur
    ...
    def obtenir_cases_ennemies(self, joueur):
        ...
        """
        # VOTRE CODE ICI
        case_joueur = self.obtenir_cases_joueur(joueur)
        dico = {}
        for case in self.cases:
            if case not in case_joueur:
                dico[case] = self.cases[case]
        return dico
    ...
    def obtenir_cases_non_pleines(self, joueur):
        ...
        case_joueur = self.obtenir_cases_joueur(joueur)
        dico={}
        for case in case_joueur:
            if not case_joueur[case].est_pleine():
               dico[case] = case_joueur[case]
        return dico
    ...
    def cases_disponibles_pour_defense(self, joueur, case_attaque):
        ...
        case_defense={}
        for cordonnees,case in self.cases.items():
            if cordonnees in self.obtenir_cases_ennemies(joueur):
                for case in case_attaque.voisins:
                    if case.coordonnees==cordonnees:
                        case_defense[case.coordonnees] = case
        return case_defense
